...
from .forms import CarreraAcademicaForm, JuntaEvaluadoraForm, EvaluacionForm, FormularioUploadForm
...
class CarreraAcademicaDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['activas'] = CarreraAcademica.objects.filter(
            estado='activa').count()
        context['en_licencia'] = CarreraAcademica.objects.filter(
            estado='licencia').count()
        ...
        hoy = date.today()
        context['proximas_vencer'] = CarreraAcademica.objects.filter(
            estado='activa',
            fecha_vencimiento_actual__lte=hoy + timedelta(days=180)
        ).select_related('cargo__docente').order_by('fecha_vencimiento_actual')[:10]
...
class CarreraAcademicaCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('carrera_academica:carrera_detail', kwargs={'pk': self.object.pk})

    def form_valid(self, form):
        messages.success(
            self.request, 'Carrera académica creada exitosamente.')
...
class CarreraAcademicaUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, 'Carrera académica actualizada exitosamente.')
        return super().form_valid(form)
...
class EvaluacionCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('carrera_academica:carrera_detail',
...
class FormularioUploadView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('carrera_academica:evaluacion_detail',
...
class ReporteVencimientosCAView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['vencidas'] = CarreraAcademica.objects.filter(
            estado='activa',
            fecha_vencimiento_actual__lt=hoy
