...
class AsignaturaParaEquivalencia(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.docente_responsable:
            ...
            cargo_principal = self.asignatura.cargo_set.filter(
                estado='activo',
                caracter__in=['ordinario', 'regular']
            ).order_by('categoria', 'fecha_inicio').first()
...
class DetalleSolicitud(models.Model):
    ...
    solicitud = models.ForeignKey(
        SolicitudEquivalencia, on_delete=models.CASCADE)
    asignatura = models.ForeignKey(
        AsignaturaParaEquivalencia, on_delete=models.CASCADE)
    estado_asignatura = models.CharField(
        max_length=50,
        choices=ESTADO_ASIGNATURA_CHOICES,
        default='pendiente'
    )
    detalle_pc = models.TextField(
        blank=True,
        help_text="Detallar qué debe cubrir el programa complementario"
    )
