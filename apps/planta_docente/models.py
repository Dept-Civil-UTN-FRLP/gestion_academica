...
class Correo(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.es_principal:
            Correo.objects.filter(docente=self.docente, es_principal=True).update(
                es_principal=False
            )
...
class Asignatura(models.Model):
    ...
    departamento = models.ForeignKey("core.Departamento", on_delete=models.PROTECT)
    carrera = models.ForeignKey("core.Carrera", on_delete=models.PROTECT)
    es_obligatoria = models.BooleanField(default=True)
    forma_dictado = models.CharField(max_length=10, choices=FORMA_DICTADO_CHOICES)
...
class Resolucion(models.Model):
    ...
    archivo_digital = models.FileField(upload_to="resoluciones/", null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.fecha_vencimiento:
            if self.caracter == "interino":
                ...
                self.fecha_vencimiento = self.fecha_inicio + relativedelta(years=anios)
