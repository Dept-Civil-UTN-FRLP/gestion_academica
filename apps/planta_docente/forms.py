...
class DocenteForm(forms.ModelForm):
    ...
    class Meta:
        model = Docente
        fields = [
            'apellido', 'nombre', 'documento', 'legajo',
            'fecha_nacimiento', 'cv'
        ]
        widgets = {
    ...
    def clean(self):
        ...
        if principales > 1:
            raise forms.ValidationError(
                'Solo puede haber un correo principal.')
...
class CargoForm(forms.ModelForm):
    ...
    class Meta:
        model = Cargo
        fields = [
            'docente', 'asignatura', 'comision', 'caracter', 'categoria',
            'dedicacion', 'cantidad_horas', 'fecha_inicio', 'resolucion_alta',
            'observaciones'
        ]
        widgets = {
...
class ResolucionForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'detalle_funciones_sustantivas': forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
            'observaciones': forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
        }
