...
class PlantaDocenteDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['vencimientos_proximos'] = cargos.filter(
            estado='activo',
            fecha_vencimiento__lte=date.today() + timedelta(days=90)
        ).select_related('docente', 'asignatura').order_by('fecha_vencimiento')[:10]
...
class DocenteUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy('planta_docente:docente_detail', kwargs={'pk': self.object.pk})
...
class AsignaturaDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['cargos_activos'] = self.object.cargo_set.filter(
            estado='activo'
        ).select_related('docente').order_by('categoria')
        return context
...
class AsignaturaUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy('planta_docente:asignatura_detail', kwargs={'pk': self.object.pk})
...
class CargoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy('planta_docente:cargo_detail', kwargs={'pk': self.object.pk})
...
class ReportePlantaCompletaView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        resumen_por_departamento = cargos_qs.values(
        ...
        context['resumen_por_depto_dict'] = {
            item['asignatura__departamento__pk']: item for item in resumen_por_departamento
        }
        ...
        context['total_docentes'] = cargos_qs.values(
            'docente').distinct().count()
        context['total_departamentos'] = cargos_qs.values(
            'asignatura__departamento').distinct().count()
        ...
        ).order_by('asignatura__departamento__nombre', 'docente__apellido', 'docente__nombre')
