...
class PSolicitud(models.Model):
    ...
    estudiante = models.ForeignKey("equivalencias.Estudiante", on_delete=models.PROTECT)
    ...
    tutor = models.ForeignKey(
        "planta_docente.Docente", related_name="tutor_ps", on_delete=models.PROTECT
    )
    supervisor = models.CharField(
        max_length=255, blank=True, help_text="Nombre del supervisor. Puede ser externo"
    )
    ...
    informe_final = models.FileField(upload_to="ps/informes/", null=True, blank=True)
    ...
    def notificar_jurados(self, tipo="plan"):
        ...
        for jurado in jurados:
            if (
                jurado.docente
                and jurado.docente.correos.filter(es_principal=True).exists()
            ):
...
class JuradoPS(models.Model):
    ...
    estado_dictamen_plan = models.CharField(
        max_length=30, choices=ESTADO_DICTAMEN_CHOICES, default="pendiente"
    )
    ...
    def clean(self):
        ...
        if not self.docente and not self.nombre_externo:
            raise ValidationError("Debe especificar un docente o un nombre externo.")
    ...
    def _actualizar_estado_solicitud(self):
        ...
        if solicitud.estado_general == "en_proceso":
            ...
            if all(
                d == "aprobado" or d == "aprobado_observaciones"
                for d in dictamenes_plan
            ):
